...
| Sender-key put, including a raised iteration lease | Encryption returns the record and distribution to the cache, then `flush` persists both as needed | Only for names included in a successful `put_sender_keys_batch` | `failed_flush_keeps_the_sender_key_gate_closed`, `only_encrypt_marked_sender_keys_gate_the_wire` |
...
| Batch failure or partial progress | Retry the still-current serialized values and tombstones, or confirm a newer value superseded them | No entry is released by the failed call; entries already confirmed by an earlier successful call may be released individually | `incomplete_session_flush_retains_newer_state_and_fails_closed_on_recovery`, `a_failed_flush_leaves_both_flags_raised` |
...
| Outbound publication | Encrypt and return state, then call `persist_signal_state_pre_wire` | The transport is reached only after a successful flush when a gate is raised | `signal_durability_chaos_smoke`, `signal_durability_sqlite_process_restart` |
