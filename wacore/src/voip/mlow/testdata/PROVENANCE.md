...
## Live capture (not synthetic)

| fixture | consumer / test | oracle |
| --- | --- | --- |
| `live_120ms_frames.json` | `decoder.rs::live_desktop_packets_decode_without_saturating` | none — 40 consecutive active 120 ms packets captured from a live WhatsApp Desktop peer and reported on #1105 |

The one fixture here that is NOT derived from `synth_mic.raw`. It exists because the synthetic
120 ms vector, generated by the C encoder, does not reproduce what the real client sends: those
packets are 37-161 bytes where the generated ones are 265-351, and only the captured ones expose the
decode divergence. Contributed as deliberately neutral speech, so it carries no personal content.

There is no reference PCM for it: the C reference decodes these packets to pure silence, so it
cannot serve as an oracle here. The test therefore asserts the coarse property that regressed —
output that does not saturate — rather than a sample-accurate target.
